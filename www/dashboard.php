...
<html>
	...
	function editar(id, txtTarefa) {
		...
		form.action = "dashboard.php?pag=dashboard&acao=atualizar";
		form.method = "POST";
		form.className = 'row'
	...
	function remover(id) {
		location.href =  '../scripts/tarefa_controller.php?pag=dashboard&acao=remover&id='+id;
	}

	function marcarRealizada(id) {
		location.href =  '../scripts/tarefa_controller.php?pag=dashboard&acao=marcarRealizada&id='+id;
	}

	function marcarPendente(id) {
		location.href =  '../scripts/tarefa_controller.php?pag=dashboard&acao=marcarPendente&id='+id;
	}
	</script>
	</head>
					...
					<div class="row">
						<div class="col">
							...
							<?php 
									...
									<div class="col-sm-3 mt-2 d-flex justify-content-between">
										<i class="fas fa-trash-alt fa-lg text-danger" style="cursor: pointer;" onclick="remover(<?= $tarefa->id?>);"></i>
											...
											<?php if($tarefa->status == 'realizado') { ?>
												<i class="fas fa-edit fa-lg text-info" style="cursor: pointer;" onclick="editar(<?= $tarefa->id?> , '<?= $tarefa->tarefa ?>');"></i>
												<i class="fas fa-clock fa-lg text-warning" style="cursor: pointer;" onclick="marcarPendente(<?= $tarefa->id?>);"></i>
											<?php } ?>
										</div>
									</div>
									<hr/>
								<?php } ?>
								<?php if($tarefas == null) {?>
									<p class="text-muted">Você não tem nenhuma tarefa cadastrada no momento. Para adicionar clique na opção  <b><i class="fas fa-plus"></i> Nova Tarefa</b>.</p>
								<?php }?>
							</div>
						</div>
					</div>
